...
public class Parser {
    ...
    private boolean seeEnhanceForStatement() {
        scanner.recordPosition();
    ...
    private JClassDeclaration classDeclaration(ArrayList<String> mods) {
        ...
        if (have(IMPLEMENTS)) {
        	implementList.add(qualifiedIdentifier().jvmName());
        	while(have(COMMA)) {
        		implementList.add(qualifiedIdentifier().jvmName());
        	}
        }
        return new JClassDeclaration(line, mods, name, superClass, 
        								classBody(), implementList);
    }
    ...
    private JInterfaceDeclaration interfaceDeclaration(ArrayList<String> mods) {
    	...
    	mustBe(IDENTIFIER);
    	String name = scanner.previousToken().image();
    	ArrayList<Type> extendList = new ArrayList<Type>();
    	if (have(EXTENDS)) {
    		extendList.add(qualifiedIdentifier());
    		while(have(COMMA)){
    			extendList.add(qualifiedIdentifier());
    		}
    	}
    	return new JInterfaceDeclaration(line, mods, name, extendList, interfaceBody());
    }
    ...
    private ArrayList<JMember> classBody() {
        ...
        while (!see(RCURLY) && !see(EOF)) {
        	scanner.recordPosition();
        	if (have(STATIC) && see(LCURLY)) {
        		int line = scanner.token().line();
        		members.add(new JStaticInstanceBlock(line, block(), true));
        	} else if (see(LCURLY)) {
        		int line = scanner.token().line();
        		members.add(new JStaticInstanceBlock(line, block(), false));
        	} else {
        		scanner.returnToPosition();
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            ...
            if (seeEnhanceForStatement()) {
            	JFormalParameter formalParameter = formalParameter();
            	mustBe(COLON);
            	expression = expression();
            	mustBe(RPAREN);
            	statements = statement();
            	return new JEnhanceForStatement(line,formalParameter,expression,statements);
            }
            if (!see(SEMI)) {
        ...
        } else if (seeLabeledStatement()) {
            mustBe(IDENTIFIER);
            String identifier = scanner.previousToken().image();
            mustBe(COLON);
            JStatement statement = statement();
            return new JLabeledStatement(line, identifier, statement);
    ...
    private JSwitchBlockStatement switchBlockStatementGroup() {
    	...
    	JStatement blockStatement = blockStatement();
    	return new JSwitchBlockStatement(line, switchLabels, blockStatement);
    }
    ...
    private JExpression unaryExpression() {
        ...
        if (have(INC)) {
        	...
        	return new JAdditionOp(line, unaryExpression());
        }
        else {
    ...
    private JExpression postfixExpression() {
        ...
        while (have(DEC) || have(INC)) {
        	if (scanner.previousToken().image() == "--")
