...
enum TokenKind {
    EOF("<EOF>"), ABSTRACT("abstract"), BOOLEAN("boolean"), CHAR("char"), CLASS(
            ...
            "("), REM("%"), RPAREN(")"), LCURLY("{"), RCURLY("}"), LBRACK("["), RBRACK(
            "]"), SEMI(";"), COMMA(","), DOT("."), IDENTIFIER("<IDENTIFIER>"), INT_LITERAL(
            "<INT_LITERAL>"), CHAR_LITERAL("<CHAR_LITERAL>"), STRING_LITERAL(
            "<STRING_LITERAL>"), ASSERT("assert"), BREAK("break"), BYTE("byte"), CASE("case"),
            CATCH("catch"), CONST("const"), CONTINUE("continue"), DEFAULT("default"), DO("do"),
            DOUBLE("double"), ENUM("enum"), FINAL("final"), FINALLY("finally"), FLOAT("float"),
            FOR("for"), GOTO("goto"), IMPLEMENTS("implements"), INTERFACE("interface"), LONG("long"),
            NATIVE("native"), SHORT("short"), STRICTFP("strictfp"), SWITCH("switch"), SYNCHRONIZED(
            "synchronized"), THROW("throw"), THROWS("throws"), TRANSIENT("transient"), TRY("try"),
            VOLATILE("volatile"), GE(">="), LT("<"), LNOTE("!="), TILDE("~"), QUES("?"), COLON(":"),
            AND("&"), AND_ASSIGN("&="), OR("|"), LOR("||"), OR_ASSIGN("|="), REM_ASSIGN("%="), 
            STAR_ASSIGN("*="), MINUS_ASSIGN("-="), DIV_ASSIGN("/="), CARET("^"), CARET_ASSIGN("^="), 
            L_SHIFT("<<"), L_SHIFT_ASSIGN("<<="), R_SHIFT(">>"), R_SHIFT_ASSIGN(">>="), UNSIGN_R_SHIFT(
            ">>>"), UNSIGN_R_SHIFT_ASSIGN(">>>="), DOUBLE_LITERAL("<DOUBLE_LITERAL>");
